...
### FBA activation no longer loops back to "Set up" (2026-09-09)

Second hitch after the catalog-error fix: Set up appeared stuck with no message. Session 8 rows switched at 13:23:56 were stamped "has not activated its FBA channel. Tap Set up again" 78 seconds later because `api_fba_prep_enable_fba_status` measured elapsed time in UTC against `_listagent_now_iso` local timestamps (always ~4 h), and the UI only showed that message on red rows. Meanwhile FBA inventory already held FNSKUs (OW-BUK2-TVB7 X005ASUE9F, XK-PQ59-TA3O X005ARVWLZ) while Listings Items still reported no FNSKU and channel DEFAULT, and `listPrepDetails` answered "MSKUs are not available for inbound … try again later". Session 7 shows the same conversion path completing in ~35 minutes.

`_fba_listing_readiness` now also reads `attributes.fulfillment_availability` (submitted channel), falls back to the FBA inventory FNSKU (`_fba_inventory_fnsku`), and classifies FNSKU-with-inbound-pending or submitted/live AMAZON_NA as `enabling` with a saved `fba_activation_note`; `ready` requires the FNSKU and a prep lookup that no longer says not available for inbound. Elapsed time uses local time. The template shows activation notes with the start time and elapsed minutes (guidance after 90 min), the "Amazon is activating" panel states the 5–40 minute expectation, and polling backs off to 20 s after 3 min and 60 s after 15 min. Deployed fba_readiness.py, fba_inventory.py, fba_prep.html (Pi backups in /tmp/fba-activation-fix-20260909), service active, /fba-prep 200. Desktop debby received identical edits plus the four new tests. Validation: 225 modular tests with only the 4 known snapshot failures; 85 inline scripts parse; FBA JS suites pass; Desktop count-scan suite passes apart from its 5 known-stale tests. Read-only Amazon checks only; no listing changes were made during verification.

### Amazon plan step lists every not-ready SKU with its reason (2026-09-09)

The plan-step panel only showed "N Seller SKUs need FBA setup. Amazon setup is ready to continue." while the reasons sat on Count-step rows as raw SP-API text; when failed and activating rows coexisted, the activating panel was hidden entirely. The panel now lists failed SKUs (barcode, title, SKU, reason, Resolve on Amazon link) and activating SKUs (activation note with start time) in separate boxes, and the misleading "ready to continue" line is gone. `_fba_friendly_enablement_error` turns NOT_FOUND into "Amazon has no listing for Seller SKU X … correct the Seller SKU in the Count step" and keeps QuotaExceeded wording retryable. Live session 8 at the time: CC-S5SD-RUW2 and K0-MGPQ-SUNX returned NOT_FOUND (stale seller SKUs; CC-S5SD-RUW2 had an FNSKU at 13:06), five SKUs activating since 14:46, two ready. Deployed fba_readiness.py and fba_prep.html (Pi backups in /tmp/fba-panel-fix-20260909); Desktop debby synced with the same edits and test. 120 modular FBA tests, 85 inline scripts, rejection-routing JS pass.

### Relisted items switch to their current seller SKU; status mirrored on page 2 (2026-09-09)

Barcode 194137216844 kept failing with NOT_FOUND for K0-MGPQ-SUNX after the user relisted it because Amazon now sells it as GA-JHZ9-YNS2 (same ASIN B0HDDM9FJW) while amazonStore.db ITEMS still mapped the UPC to the dead SKU, so every rescan resolved to it. `api_fba_prep_enable_fba` now catches NOT_FOUND, calls `_fba_replacement_seller_sku` (searchListingsItems by ASIN, then UPC, excluding the stale SKU, preferring BUYABLE/DISCOVERABLE), rewrites the session row's seller SKU, records the switch in `fba_listing_notes`, updates the cache through `_fba_remember_replacement_listing`, and continues setup; several live candidates produce an explicit "type the right Seller SKU" message. `setStatus` now writes to both `#statusBar` (Count page) and the new `#amazonStatusBar` in the Amazon card, so Set up feedback appears on page 2. Deployed fba_readiness.py, fba_inventory.py, fba_prep.html (Pi backups in /tmp/fba-relist-fix-20260909); Desktop debby received equivalent edits (helper inserted before `_fba_local_amazon_listing`) and tests. 122 modular FBA tests, 85 inline scripts, rejection-routing JS pass.

### Step 3 "Print label only" scan mode (2026-09-09)

Packing step has an optional toggle "Print label only — no box sorting" (shown only while the packing scanner is active and not in carton-move mode; default off on every page load). With it on, `scanItem` routes to `printLabelOnlyForScan`, which resolves the planned row, applies the same rejection guard, requires the plan to call for a seller FNSKU label (manufacturer-barcode items get a warning instead), and calls the existing `/amazon/print-item-label` endpoint. No pack-scan request, carton assignment, or warehouse removal happens; printed counts still increment. The scan-mode label, button, placeholder, callout, and voice say "label only / not packed". Carton-move mode takes precedence. Client-side only; no backend change. Added test_fba_label_only.cjs (dispatch, printing, no packing, move precedence, no-label and rejection guards) to both checkouts; deployed fba_prep.html to the Pi (backup /tmp/fba-label-only-20260909), service active, page verified.
