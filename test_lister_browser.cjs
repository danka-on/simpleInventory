...
(async () => {
  ...
  try {
    ...
    // One click selects a row, a click on the selected row lists it.
    const startRow = async upc => { const sel = `.item[data-upc="${upc}"]`; if (!(await panel.$eval(sel, el => el.classList.contains('armed')))) { await panel.click(sel); await panel.waitForSelector(sel + '.armed'); } await panel.click(sel); };
    ...
    assert.equal(await panel.$eval('.item.armed', el => el.dataset.upc), UPC + '-1', 'one click selects the row and stays on the queue');
    assert.ok(await panel.$eval('#crumb', el => el.hidden), 'one click does not open the item');
    await panel.click('.item[data-upc="883049370897-1"] button[data-open]');
    ...
    await toQueue();
    await toQueue();
    await startRow('012345678905');
    await store.waitForURL(/\/sl\/prelist\/identify\?sr=sug&title=012345678905/, { timeout: 20000 });
    // Selecting the row and clicking it again starts it: the store tab goes back to eBay's start page and searches that UPC.
    await store.goto('https://www.ebay.com/sl/prelist/suggest?sr=wn');
    await toQueue();
    await startRow('012345678905');
    await store.waitForURL(/\/sl\/prelist\/identify\?sr=sug&title=012345678905/, { timeout: 20000 });
    // Amazon works the same: a second click on the row opens Seller Central's product search (/abis/listing/syh
    ...
    // Already on that page: a second click searches right there, no reload.
    ...
    await panel.bringToFront();
    // ⋯ -> Options on the list: the switches and prompts are set before any item opens.
    await panel.click('#moreBtn');
    await panel.click('#modal .menu-item:has-text("Options")');
    await panel.waitForSelector('#optAuto input#o_autoAiTitle');
    assert.ok(await panel.$('#optAiPrompt') && await panel.$('#optTitlePrompt'), 'the AI prompts are there too');
    const titleWas = await panel.$eval('#o_autoAiTitle', el => el.checked);
    await panel.click('#optAuto label:has(#o_autoAiTitle)');
    await panel.waitForFunction(was => document.getElementById('o_autoAiTitle')?.checked === !was, titleWas);
    const saved = await panel.evaluate(() => chrome.storage.local.get(null));
    assert.ok(Object.values(saved).some(v => v && typeof v === 'object' && v.autoAiTitle === !titleWas), 'the switch saves the same setting');
    await panel.click('#optAuto label:has(#o_autoAiTitle)');
    await panel.waitForFunction(was => document.getElementById('o_autoAiTitle')?.checked === was, titleWas);
    await panel.click('#optClose');
    await panel.waitForFunction(() => document.getElementById('modal').hidden && !document.getElementById('optAuto'));
