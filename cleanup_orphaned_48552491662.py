import sqlite3

print("=" * 80)
print("CLEANING UP ORPHANED SUFFIXED ENTRIES FOR UPC 48552491662")
print("=" * 80)

conn = sqlite3.connect('bol.db')
cur = conn.cursor()

# Find orphaned suffixed entries (temporary=1, all quantities zero)
cur.execute('''
    SELECT id, upc, lot_number, temporary, original_qty, good_qty, bad_qty, unchecked_qty
    FROM bol_items 
    WHERE upc LIKE '48552491662-%'
    AND temporary = 1
    AND (original_qty = 0 OR original_qty IS NULL)
    AND (good_qty = 0 OR good_qty IS NULL)
    AND (bad_qty = 0 OR bad_qty IS NULL)
    AND (unchecked_qty = 0 OR unchecked_qty IS NULL)
''')

orphaned = cur.fetchall()

print(f"\nFound {len(orphaned)} orphaned suffixed entries:")
print("-" * 80)

for row in orphaned:
    id_val, upc, lot, temp, orig, good, bad, unch = row
    print(f"ID {id_val}: {upc} (LOT {lot}) - All quantities zero, temporary={temp}")

if not orphaned:
    print("No orphaned entries found.")
    conn.close()
    exit(0)

response = input(f"\nDelete these {len(orphaned)} orphaned entries? (yes/no): ").strip().lower()

if response == 'yes':
    for row in orphaned:
        id_val = row[0]
        upc = row[1]
        
        # Delete from bol_items
        cur.execute('DELETE FROM bol_items WHERE id = ?', (id_val,))
        print(f"  ✓ Deleted {upc} (ID {id_val})")
        
        # Also delete any related prep data
        cur.execute('DELETE FROM items_prep_status WHERE upc = ?', (upc,))
        cur.execute('DELETE FROM items_prep_images WHERE upc = ?', (upc,))
        try:
            cur.execute('DELETE FROM items_prep_notes WHERE upc = ?', (upc,))
        except:
            pass
    
    conn.commit()
    
    print(f"\n✅ Deleted {len(orphaned)} orphaned entries")
    
    # Verify cleanup
    cur.execute('SELECT COUNT(*) FROM bol_items WHERE upc LIKE ?', ('48552491662%',))
    remaining = cur.fetchone()[0]
    print(f"✓ Remaining entries for UPC 48552491662: {remaining}")
    
else:
    print("❌ Cleanup cancelled")

conn.close()
