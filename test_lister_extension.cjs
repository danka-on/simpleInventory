...
assert.equal(M.detectPage('https://sellercentral.amazon.com/abis/listing/syh?ref_=xx_addprod_dnav_xx').kind, 'listing-start', '"List Your Products" without a product is the search step');
assert.equal(M.detectPage('https://sellercentral.amazon.com/abis/listing/syh?asin=B0TESTASIN').kind, 'offer-form');
