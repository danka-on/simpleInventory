...
(async () => {
  ...
  try {
    ...
    await page.getByRole('tab',{name:'Out of stock (0)',exact:true}).click();
    ...
    assert.equal(await page.getByRole('button',{name:'Search whole warehouse with Claude',exact:true}).count(),0,'Claude search is no longer offered per row');
    failLink=true;
    ...
    await page.getByRole('tab',{name:'All active listings (1)',exact:true}).click();
